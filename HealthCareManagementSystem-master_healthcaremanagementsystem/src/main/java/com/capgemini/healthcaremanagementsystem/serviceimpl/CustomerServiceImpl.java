...
public class CustomerServiceImpl implements CustomerService {
	...
	public Appointment makeAppointment(Appointment appointment) {
		Long centerId=appointment.getCenterId();
		Optional<DiagnosticCenter> option=diagnosticCenterDao.findById(centerId);
		DiagnosticCenter dc;
		if(option.isPresent()) {
			 dc=option.get();
		}	
		else {
			throw new DiagnosticCenterNotExistsException("center is not found with thid id");
		}
		
		List<Appointment> list = dc.getAppointments();
